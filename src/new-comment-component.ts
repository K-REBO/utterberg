...
import { CLIENT_ID } from './utterberg-config';
...
export class NewCommentComponent {
  ...
  constructor(
    ...
    this.element.innerHTML = `
      ...
      <form class="comment" accept-charset="UTF-8" action="javascript:">
        ...
        <footer class="new-comment-footer">
          ...
          <button class="btn btn-primary" type="button" id="utterberg-sign-in">
    ...
    this.signInButton = this.form!.lastElementChild!.lastElementChild! as HTMLButtonElement;
    this.submitButton = this.signInButton.previousElementSibling! as HTMLButtonElement;
  ...
  private handleSignIn = async () => {
    if (!CLIENT_ID) {
      alert('utterberg: client-id が設定されていません。\nCodeberg OAuth Appを登録してclient-idを指定してください。');
      return;
    }
    const loginUrl = await getLoginUrl(page.url);
