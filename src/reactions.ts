import { toggleReaction, ReactionID, reactionTypes } from './codeberg';
  ...
  <details class="details-overlay details-popover reactions-popover">
    ...
    <div class="Popover" style="${position}">
      <div class="Popover-message ${alignmentClass} box-shadow-large" style="padding: 16px">
        <span><button class="btn-link utterberg-sign-in-react" type="button">Sign in with Codeberg</button> to add your reaction.</span>
      </div>
    </div>
  </details>`;
}

// サインインリアクションボタンのクリックハンドラーを有効化する
export function enableSignInToReact() {
  document.addEventListener('click', async (event) => {
    const btn = (event.target as HTMLElement).closest('.utterberg-sign-in-react');
    if (!btn) return;
    const loginUrl = await getLoginUrl(pageAttributes.url);
